        ...
        config = {
            ...
            'BIPED': {'img_height': 720,  # 720
                    ...
                    #   'train_list': 'train_pair0.lst',
                      'train_list': 'train_pair.lst',
                      'data_dir': '../../../Downloads/archive/BIPEDv2/BIPEDv2/BIPED',  # WIN: '../.../dataset/BIPED/edges'
                    #   this one for biped
                      'yita': 0.5,
                      'mean':BIPED_mean},
            'BIPED-B2': {'img_height': 720,  # 720
...
class TestDataset(Dataset):
    def __init__(self,
        ...
        self.args = arg
        self.up_scale = False if arg is None else arg.up_scale
        self.mean_bgr = arg.mean_test if arg is not None and len(arg.mean_test) == 3 else [104.007, 116.669, 122.679] if arg is None else arg.mean_test[:3]
    ...
    def _build_index(self):
        sample_indices = []
        if self.test_data == "CUSTOM":
            # for single image testing
            labels_path = None
            if os.path.isdir(self.data_root):
                # если self.data_root - каталог
                images_path = [file for file in os.listdir(self.data_root) if not file.endswith('.DS_Store')]
            elif os.path.isfile(self.data_root):
                # если self.data_root - файл
            ...
            if self.test_data.upper() in ['BIPED', 'BRIND','UDED','ICEDA']:
                ...
                for pair in pairs:
                    tmp_img = pair[0]
                    tmp_gt = pair[1]
                    if not tmp_img.endswith('.DS_Store') and not tmp_gt.endswith('.DS_Store'):
                        sample_indices.append(
                            (os.path.join(self.data_root, tmp_img),
                                os.path.join(self.data_root, tmp_gt),))
    ...
    def __len__(self):
        return len(self.data_index[0]) if self.test_data.upper() == 'CUSTOM' else len(self.data_index)
    ...
    def __getitem__(self, idx):
        ...
        label_path = None if self.test_data == "CUSTOM" else self.data_index[idx][1]
        ...
        elif self.test_data.upper() == 'CUSTOM':
...
class BipedDataset(Dataset):
    ...
    def transform(self, img, gt):
        ...
        if i_w> 400 and i_h>400:
            h,w = gt.shape
            if np.random.random() > 0.4:

                LR_img_size = crop_size
